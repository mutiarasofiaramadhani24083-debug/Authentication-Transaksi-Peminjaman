<x-app-layout>
    ...
    <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded-lg shadow-sm flex items-center justify-between">
        <div>
            <h3 class="text-red-800 font-bold text-lg">⚠️ Perhatian: Buku Terlambat</h3>
            <p class="text-red-600 text-sm">Ada <strong>{{ \App\Models\Transaksi::where('status', 'Dipinjam')->get()->filter(fn($t) => $t->terlambat > 0)->count() }}</strong> transaksi yang telah melewati batas pengembalian.</p>
        </div>
        <a href="{{ route('transaksi.index') }}" class="bg-red-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-red-700 transition">
            Lihat Detail
        </a>
    </div>
</div>
